...
#### 2a. The share/work correlation: measured false, tolerated

The share above is a fraction of the search *tree*. Everything downstream — the
cost blend in §3, and `ut/finished - ut` in the GUI — treats it as a fraction of
the *work*. That assumption was carried as an open question through
implementation. It has now been measured, and it is **false**.

`generateSubtreeTasks` drops subtrees it proves empty at the cutoff depth and
folds their share into `prunedTaskShare`, which is seeded into `completedShare`
*before any worker starts*. Proving those subtrees empty costs microseconds;
the share they carry is credited in full. Measured directly, at the depth
(`<= 3`) and budget (`max(16, 4 × threads)`) `parallelMultiSearch` actually
uses, over every bundled example `assembler_0_c` accepts:

| puzzle | seeded `prunedTaskShare` |
| :--- | ---: |
| `DiagonalCube` | **0.5** |
| `BallRoom` | 0.125 |
| all others (incl. `Burr-Glar`) | 0 |

The value does not move with the thread count (1 through 32) or with the
`createMatrix` flags. So a fresh parallel solve of `DiagonalCube` opens with
`getFinished()` at 0.5 before a single node of real search has run, and the
GUI's time estimate under-reports the remaining time by roughly 2× through the
early solve.

This is the *same* anti-correlation that was measured for `assembler_1_c` and
that got structural weighting **rejected** there: its tasks are generated by
running the real search to the cutoff, so on `Burr-Glar` the weighting handed
instantly dead subtrees 96.9% of the weight inside the first 10 ms. The good
measured curve for `assembler_0_c` on `Burr-Glar` is therefore **not** evidence
for the weighting — `Burr-Glar` prunes nothing at this depth, so it never
exercises it. The curve is the in-flight term of §2 doing the work.

It is **tolerated** here, and shipped, for three reasons:

1. The scale is an order of magnitude apart — half a tree credited once at
   `t=0`, against 96.9% credited within 10 ms of a 315 s solve.
2. The scheme stays share-conserving: nothing is double-counted, the bar stays
   monotone, and it still ends at exactly 1.0.
3. It replaced 106 discrete steps that froze for tens of seconds at a time.
   A bar that starts optimistically and then moves smoothly is strictly better
   than one that is accurate and then stops.

Removing the bias means not crediting pruned subtrees structurally — weighting
by measured cost rather than by tree shape, which is a redesign of §2, not a
tuning constant. Recorded, not modelled. `assembler_0.h`'s `prunedTaskShare`
comment carries the same record at the code.
...
- **`N̂` is only as good as `a`.** If the assembly fraction is itself skewed,
  ...
  This risk is **realised**, not hypothetical: §2a measures `a` opening at 0.5
  on `DiagonalCube`. Accepted — see §2a for why it ships anyway.
