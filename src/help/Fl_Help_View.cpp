...
#else
//              fl_font(FL_SYMBOL, fsize);
              fl_draw("-", xx - fsize + x() - leftline_, yy + y());
#endif
	    }
...
Fl_Help_View::get_image(const char *name, int W, int H) {
  ...
  static std::vector<Fl_Image*> cache;

  int i = 0;

  while (imagelist[i].name && strcmp(imagelist[i].name, name)) i++;

  Fl_Image *ip = 0;

  if (imagelist[i].name) {

    // make sure cache has the required size
    while (cache.size() < i+1)
      cache.push_back(0);

    if (cache[i] == 0)
      cache[i] = new Fl_RGB_Image(imagelist[i].data, imagelist[i].w, imagelist[i].h);
