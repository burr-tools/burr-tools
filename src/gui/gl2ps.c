...
static void gl2psParseFeedbackBuffer(GLint used)
{
  ...
  while(used > 0){
    ...
    case GL_LINE_RESET_TOKEN :
      ...
      // PATCH FOR BURRTOOLS
      if (vertices[0].xyz[0] < gl2ps->boundMinX) gl2ps->boundMinX = vertices[0].xyz[0];
      if (vertices[0].xyz[0] > gl2ps->boundMaxX) gl2ps->boundMaxX = vertices[0].xyz[0];
      if (vertices[0].xyz[1] < gl2ps->boundMinY) gl2ps->boundMinY = vertices[0].xyz[1];
      if (vertices[0].xyz[1] > gl2ps->boundMaxY) gl2ps->boundMaxY = vertices[0].xyz[1];

      if (vertices[1].xyz[0] < gl2ps->boundMinX) gl2ps->boundMinX = vertices[1].xyz[0];
      if (vertices[1].xyz[0] > gl2ps->boundMaxX) gl2ps->boundMaxX = vertices[1].xyz[0];
      if (vertices[1].xyz[1] < gl2ps->boundMinY) gl2ps->boundMinY = vertices[1].xyz[1];
      if (vertices[1].xyz[1] > gl2ps->boundMaxY) gl2ps->boundMaxY = vertices[1].xyz[1];
    ...
    case GL_POLYGON_TOKEN :
      ...
      while(count > 0 && used > 0){
        ...
        // PATCH FOR BURRTOOLS
        if (vertices[v].xyz[0] < gl2ps->boundMinX) gl2ps->boundMinX = vertices[v].xyz[0];
        if (vertices[v].xyz[0] > gl2ps->boundMaxX) gl2ps->boundMaxX = vertices[v].xyz[0];
        if (vertices[v].xyz[1] < gl2ps->boundMinY) gl2ps->boundMinY = vertices[v].xyz[1];
        if (vertices[v].xyz[1] > gl2ps->boundMaxY) gl2ps->boundMaxY = vertices[v].xyz[1];
...
static void gl2psPrintPostScriptHeader(void)
{
  ...
  // PATCH BURRTOOLS add a content box to the EPS
  gl2psPrintf("%%%%ContentBox: %d %d %d %d\n",
              gl2ps->boundMinX,
              gl2ps->boundMinY,
              gl2ps->boundMaxX,
              gl2ps->boundMaxY);
                                  ...
                                  FILE *stream, const char *filename)
{
  ...
  // PATCH BURRTOOLS
  gl2ps->boundMinX = gl2ps->boundMinY = 10000;
  gl2ps->boundMaxX = gl2ps->boundMaxY = -10000;
