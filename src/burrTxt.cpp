...
public:
  ...
  void considerLevel(const separation_c * da) {

    std::string lev = da->movesText();

    int level = 0;
    int totalmoves = 0;
    if (!parse_dotlevel(lev.c_str(), &level, &totalmoves))
      return;

    if (!hasBest || level_is_better(level, totalmoves, bestLevel, bestTotalmoves)) {
      hasBest = true;
      strncpy(bestDotlevel, lev.c_str(), sizeof(bestDotlevel));
