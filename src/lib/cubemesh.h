...
namespace cubeMesh {
   ...
   * outward-wound triangles in cell units, welded. The gap g is per
   * body (bodies end up 2g apart), r the bevel leg. Returns false with
   * a message when the parameters are outside what the table covers
   * (g + r too large for the dual-cell construction). fills selects the
   * variant: with the interior (concave) chamfers, or without. */
