...
public:
  ...
  node1_c(int pn) : comefrom(0), piecenumber(pn) {
   ...
   */
  bool operator == (const node1_c &b) const {
  ...
  node1_c * getComefrom(void) const {
    return comefrom;
  }

  void setComefrom(node1_c *n) {
    comefrom = n;
  }
 ...
 */
class node_ptr_less : public std::binary_function<node1_c *, node1_c *, bool> {

public:

  bool operator()(const node1_c * a, const node1_c * b) const {
    return *a < *b;
  }
};
    ...
    for (int d1 = d##d1##1; d1 <= d##d1##2; d1++)        \
      for (int d2 = d##d2##1; d2 <= d##d2##2; d2++) {    \
        ...
        case 1:                                          \
          value = 32000;                                 \
          dir = d##dir##1;                               \
          search_start(dir, d##dir##2, p1);              \
          search_normal(dir, d##dir##2, p1, p2);         \
          break;                                         \
        case 2:                                          \
          value = 32000;                                 \
          dir = d##dir##1;                               \
          search_start(dir, d##dir##2, p1);              \
          search_normal(dir, d##dir##2, p1, p2);         \
          search_forward_stop(dir, c##dir##2, p2);       \
          break;                                         \
        case 4:                                          \
          dir = d##dir##1-1;                             \
          value = 0;                                     \
          search_back(dir, c##dir##1, p1);               \
          if (dir < c##dir##1) value = 32000;            \
          dir = d##dir##1;                               \
          search_normal(dir, d##dir##2, p1, p2);         \
          break;                                         \
        case 5:                                          \
        ...
        case 8:                                          \
          ...
          if (dir >= c##dir##1) {                        \
                                                         \
            dir = d##dir##1;                             \
            search_forward(dir, c##dir##2, p2);          \
            if (dir <= c##dir##2) {                      \
                                                         \
              value += d##dir##1 - d##dir##2 - 1;        \
                                                         \
              if (value < minimum)                       \
                minimum = value;                         \
            }                                            \
...
// find out which of the 6 possible arrangements we have
#define calc_arrangement(dir, Dir)                                                           \
  if (b##dir##1[pj] + searchnode->get##Dir(j) >= b##dir##1[pi] + searchnode->get##Dir(i))    \
  if (b##dir##1[pj] + searchnode->get##Dir(j) <= b##dir##2[pi] + searchnode->get##Dir(i))    \
        arrangement = 3;                                                                     \
      else                                                                                   \
        arrangement = 6;                                                                     \
    else                                                                                     \
      arrangement = 0;                                                                       \
                                                                                             \
  if (b##dir##2[pj] + searchnode->get##Dir(j) >= b##dir##1[pi] + searchnode->get##Dir(i)) {  \
    if (b##dir##2[pj] + searchnode->get##Dir(j) <= b##dir##2[pi] + searchnode->get##Dir(i))  \
        arrangement += 1;                                                                    \
      else                                                                                   \
        arrangement += 2;                                                                    \
    }

// if we exchange 2 pieces the arrangement number switches in the way done here
#define switch_arrangement()       \
  switch(arrangement) {            \
  case 0: arrangement = 8; break;  \
  case 8: arrangement = 0; break;  \
  case 2: arrangement = 4; break;  \
  case 4: arrangement = 2; break;  \
  case 1: arrangement = 5; break;  \
  case 5: arrangement = 1; break;  \
  }
 ...
 */
void disassembler_1_c::prepare(int pn, voxel_type * pieces, node1_c * searchnode) {
...
void disassembler_1_c::prepare_rec(int pn, int d, int a, int b) {
 ...
 * is rubbish.
 */
bool disassembler_1_c::checkmovement(void) {
...
void disassembler_1_c::init_find(node1_c * nd, int piecenumber, voxel_type * pieces) {
 ...
 */
node1_c * disassembler_1_c::find(node1_c * searchnode) {
 ...
 */
static void create_new_params(node1_c * st, node1_c ** n, voxel_type ** pn, int piecenumber, voxel_type * pieces, int part, bool cond) {
 ...
 */
separation_c * disassembler_1_c::disassemble_rec(int piecenumber, voxel_type * pieces, node1_c * start) {

  std::queue<node1_c *> openlist;
  std::set<node1_c *, node_ptr_less> closed;
  ...
  while (!openlist.empty()) {
    ...
    while ((st = find(node))) {
      ...
      std::set<node1_c *, node_ptr_less>::iterator i;
...
void disassembler_1_c::calcbounds(void) {
...
disassembler_1_c::~disassembler_1_c() {
  delete [] movement;
  delete [] check;
  for (int i = 0; i < 3; i++)
    delete [] matrix[i];
...
separation_c * disassembler_1_c::disassemble(void) {

  /* create the first node with the start state
   * here all pieces are at position (0; 0; 0)
   */
  node1_c * start = new node1_c(piecenumber);
