...
public:
  ...
  nodeWithMatrix_c(int pn) : comefrom(0), piecenumber(pn) {
   ...
   */
  bool operator == (const nodeWithMatrix_c &b) const {
  ...
  nodeWithMatrix_c * getComefrom(void) const {
    return comefrom;
  }

  void setComefrom(nodeWithMatrix_c *n) {
    comefrom = n;
  }
 ...
 */
class node_ptr_less : public std::binary_function<nodeWithMatrix_c *, nodeWithMatrix_c *, bool> {

public:

  bool operator()(const nodeWithMatrix_c * a, const nodeWithMatrix_c * b) const {
    return *a < *b;
  }
};
