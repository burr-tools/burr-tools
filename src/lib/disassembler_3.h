...
#ifndef __DISASSEMBLER_3_H__
#define __DISASSEMBLER_3_H__
 ...
 * need to know it exaclty, so it's defined in the c-file
 */
class nodeWithMatrix_c;

/* idea, it should be possible to update only a part of the matrix, with the
 * pieces that have moved from one state to another, this should speed
 * up the whole process considerably because the generation of the matrix
 * is quadratic, which is a bad thing, SM takes ages to disassemble...
 *
 * this requires that states that are in the open list save their matrix bevore
 * doing the 2nd part of Bills algorithm but that can be deleted after the
 * node is observed
 */
 ...
 */
class disassembler_3_c : public disassembler_c {
...
private:
   ...
   */
  nodeWithMatrix_c * find(nodeWithMatrix_c * searchnode);
   ...
   */
  separation_c * disassemble_rec(int piecenumber, voxel_type * pieces, nodeWithMatrix_c * start);

public:

  disassembler_3_c(voxel_c * prob, int piecenum);
  ~disassembler_3_c();
